    ...
    prompts: {
      ...
      sections: [
        { title: 'Core Principles', items: [
          { cmd: 'Be specific and explicit', desc: 'Vague prompts get vague answers' },
          { cmd: 'Provide context and background', desc: 'Model doesn\'t know your situation' },
          { cmd: 'Show examples (few-shot)', desc: 'Examples often beat instructions' },
          { cmd: 'Break complex tasks into steps', desc: 'Chain-of-thought improves reasoning' },
          { cmd: 'Specify the desired format', desc: 'JSON, markdown, bullet list, etc.' },
          { cmd: 'State constraints and limitations', desc: 'What to avoid, max length, language' },
          { cmd: 'Assign a persona/role', desc: '"Act as a senior backend engineer…"' },
        ]},
        { title: 'Prompt Structure', items: [
          { cmd: 'Role → Task → Context → Format → Constraints', desc: 'Effective ordering template' },
          { cmd: 'System prompt: role and persistent rules', desc: 'What the model always knows' },
          { cmd: 'User prompt: specific request', desc: 'What you want this time' },
          { cmd: 'Put instructions BEFORE content to analyze', desc: 'Models read top-to-bottom' },
          { cmd: 'Use XML tags to delimit sections', desc: '<context>...</context> prevents confusion' },
          { cmd: 'Repeat key constraints at the end', desc: 'Recency bias helps' },
        ]},
        { title: 'Techniques', items: [
          { cmd: '"Think step by step"', desc: 'Chain-of-thought reasoning' },
          { cmd: '"Before answering, ask me any clarifying questions"', desc: 'Avoid assumptions' },
          { cmd: '"Show your work / reasoning"', desc: 'Makes reasoning visible and correctable' },
          { cmd: '"Here are 3 examples: [examples]. Now do: [task]"', desc: 'Few-shot prompting' },
          { cmd: '"Draft 3 options, then choose the best"', desc: 'Self-evaluation pattern' },
          { cmd: '"What would a [expert] say about this?"', desc: 'Expert persona activation' },
          { cmd: '"Review your answer for errors before responding"', desc: 'Self-critique prompt' },
          { cmd: '"If you don\'t know, say so — don\'t guess"', desc: 'Reduce hallucinations' },
        ]},
        { title: 'Code Prompts', items: [
          { cmd: '"Write [language] code that [task]. Include error handling."', desc: 'Basic code request' },
          { cmd: '"Refactor this code for readability, keep behavior identical"', desc: 'Safe refactor' },
          { cmd: '"Add tests for the edge cases in this function"', desc: 'Test generation' },
          { cmd: '"Explain this code to a junior developer"', desc: 'Documentation/explanation' },
          { cmd: '"Find bugs in this code and explain why each is a bug"', desc: 'Code review' },
          { cmd: '"Convert this pseudocode to working [language]"', desc: 'Implementation from spec' },
          { cmd: '"What are the security vulnerabilities in this?"', desc: 'Security audit' },
        ]},
        { title: 'Claude-specific', items: [
          { cmd: '"Be concise — one sentence max per point"', desc: 'Control verbosity' },
          { cmd: '"Do not add caveats, disclaimers, or hedge language"', desc: 'Direct answers' },
          { cmd: '"This is for [authorized testing/educational] purposes"', desc: 'Context for dual-use tasks' },
          { cmd: '"Do not change anything outside of what I asked"', desc: 'Scope control' },
          { cmd: '"Return ONLY the code, no explanation"', desc: 'Extract just the artifact' },
          { cmd: '"Use the exact variable names from the existing code"', desc: 'Style consistency' },
          { cmd: '"Confirm your understanding before starting"', desc: 'Alignment check on complex tasks' },
        ]},
        { title: 'Avoiding Pitfalls', items: [
          { cmd: 'Avoid: "Make it better" → Say HOW to make it better', desc: 'Specificity beats vagueness' },
          { cmd: 'Avoid: long chat history for new tasks → Start fresh', desc: 'Context pollution' },
          { cmd: 'Avoid: multiple unrelated tasks in one prompt', desc: 'One task = better focus' },
          { cmd: 'Avoid: "Write me a complete app" without specs', desc: 'Underspecified = unusable output' },
          { cmd: 'Check: does your prompt assume knowledge the model lacks?', desc: 'Close the context gap' },
          { cmd: 'Iterate: first output informs second prompt', desc: 'Treat it as a conversation' },
        ]},
      ]
    },
  };
