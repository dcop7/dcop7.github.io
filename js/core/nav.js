const Nav = (function () {
  ...
  const ICONS = {
    ...
    receitas:    svg('<path d="M3 11h18"/><path d="M12 11V3"/><path d="M7 11a5 5 0 0 1 10 0"/><path d="M5 11v3a7 7 0 0 0 14 0v-3"/><line x1="7" y1="21" x2="17" y2="21"/>'),
    ...
    sb.innerHTML = `
      <div class="sb-scroll">
        <nav class="sb-main-nav">
          ...
          <a class="sb-nav-item" data-route="receitas"     href="#receitas">${ICONS.receitas}<span>${TN('nav.receitas')}</span></a>
  ...
  function renderView(raw) {
    ...
    if (page === 'cheatsheets') {
      ...
      document.getElementById('view-receitas')?.classList.add('active');
      typeof ReceitasPage !== 'undefined' && ReceitasPage.show(sub || null);
  ...
  document.addEventListener('langchange', () => {
    const routes = ['home','links','tools','cheatsheets','games','quiz','explorer','ocorrencias','receitas','media','workout','photography','visual','settings'];
    routes.forEach(r => {
